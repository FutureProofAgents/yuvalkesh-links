...
function page(res,status,title,text){res.setHeader('Cache-Control','no-store');res.setHeader('Content-Type','text/html; charset=utf-8');return res.status(status).send(`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${title}</title><style>body{margin:0;background:#000;color:#e7e9ea;font:16px/1.6 Inter,Arial,sans-serif;display:grid;min-height:100vh;place-items:center}.card{width:min(560px,calc(100% - 40px));border:1px solid #2f3336;border-radius:20px;background:#101214;padding:36px}h1{color:#fff;font-size:28px}a{color:#1d9bf0}</style><main class="card"><h1>${title}</h1><p>${text}</p><a href="https://futureproofagents.com/newsletter/">FutureProof Newsletter</a></main></html>`);}
